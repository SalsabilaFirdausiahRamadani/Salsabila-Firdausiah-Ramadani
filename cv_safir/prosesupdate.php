...
if (isset($_POST['id'])) {
    ...
    if (isset($_SESSION['id'])) {
        ...
        $sql = 'UPDATE cvdatabase SET nama=?, alamat=?, telepon=?, email=?, web=?, pendidikan=?, pengalaman=?, keterampilan=?, foto_path=? WHERE id=?';

        $stmt = $conn->prepare($sql);
        $stmt->bind_param('sssssssssi', $nama, $alamat, $telepon, $email, $web, $pendidikan, $pengalaman, $keterampilan, $foto_path, $id);
